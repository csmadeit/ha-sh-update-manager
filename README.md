...
| Version | Changes |
|---------|---------|
| 2.1.4 | **Z-Wave Firmware queue now includes battery devices by default.** Previous default was `battery_handling=exclude` (mains-only), which silently filtered battery Z-Wave devices like the ZSE44 out of scans. Default is now `include`. Config-entry schema bumped to v6 with a v5→v6 migration that flips the stock Z-Wave Firmware queue from `exclude` to `include` on upgrade (only matches the stock queue by name + zwave integration; user-customized queues are left alone). Setup-wizard label for queue_1 updated accordingly. |
